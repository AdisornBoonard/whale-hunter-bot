...
st.set_page_config(page_title="Whale Hunter V7.5 - Virtual Display Perfect", layout="wide", initial_sidebar_state="collapsed")
...
# ดึงค่า TP / SL ที่พี่เซ็ตไว้มาใช้เป็น Default และโชว์บนหน้าจอ
...
<div style="background-color: #12161f; border: 1px solid #1e2533; padding: 10px 20px; border-radius: 6px; margin-bottom: 20px;">
    <span style="font-size: 20px; font-weight: bold; color: white;">WHALE HUNTER V7.5 - DISPLAY TP/SL TRUE</span>
...
with col_left:
    ...
    with st.container(border=True):
        ...
        # ผูกตัวแปรสไลเดอร์เข้ากับความจำหลักระบบ
...
with col_center:
    ...
    # --- ตารางแสดงออเดอร์รายไม้แบบเพิ่มการโชว์ราคาเป้าหมาย TP / SL จริงตามสูตร ---
    ...
    if virtual_orders_list:
        for order in virtual_orders_list:
            ...
            # คำนวณ % และหาราคาเส้นชัย TP และราคาคัต SL ของแต่ละไม้เดี่ยว ๆ ออกมาโชว์พี่
            ...
            with st.container(border=True):
                # ปรับคอลัมน์กางช่องเพื่อเพิ่มพื้นที่โชว์ตัวเลขราคาเป้าหมาย
                ...
                # เพิ่มบรรทัด Target TP / SL ชัดเจนตรงช่องข้อมูลราคา
...
with col_right:
    ...
    with st.container(border=True):
        ...
        st.caption(f"• ปรับแต่งกลยุทธ์: TP {st.session_state.tp_percent}% / SL {st.session_state.sl_percent}%")
